...
void FrameBuffer::Init()
{
    ...
    glGenTextures(1, &idVertexBuffer);
    glGenTextures(1, &idDepthBuffer);
}

void FrameBuffer::Resize(unsigned int w, unsigned int h, bool useLinearMode, bool usevertexBuffer){
	if (useLinearMode){
	...
	if (usevertexBuffer){
		glBindTexture(GL_TEXTURE_2D, idVertexBuffer);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, w, h, 0, GL_RGBA, GL_FLOAT, NULL);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	}
	...
	if (usevertexBuffer)
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, idVertexBuffer, 0);
...
void FrameBuffer::Destroy()
{
	...
	glDeleteTextures(1, &idVertexBuffer);
